  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <section className="relative w-full h-[600px] flex items-center justify-center bg-muted overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-primary/80 to-accent/80 mix-blend-multiply z-10" />
        <div className="absolute inset-0 flex items-center justify-center text-muted/20 text-9xl">
          HERO IMAGE
        </div>
        ...
        <div className="relative z-20 text-center text-white px-6 md:px-12 lg:px-20">
          ...
          <div className="flex gap-4 justify-center">
            <Link href="/products" className={buttonVariants({ size: "lg", className: "bg-white text-primary hover:bg-white/90" })}>
              Explore Products <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
            <Link href="/contact" className={buttonVariants({ variant: "outline", size: "lg", className: "text-white border-white hover:bg-white/20" })}>
              Contact Us
            </Link>
          </div>
        </div>
      ...
      <section className="py-24 container mx-auto px-6 md:px-12 lg:px-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { title: "Agriculture", icon: Leaf, color: "text-green-500", bg: "bg-green-500/10" },
            { title: "Forestry", icon: Trees, color: "text-amber-600", bg: "bg-amber-600/10" },
            { title: "Seafood", icon: Fish, color: "text-blue-500", bg: "bg-blue-500/10" }
          ].map((cat) => (
            <Link key={cat.title} href={{ pathname: '/products', query: { category: cat.title.toLowerCase() } }}>
              <Card className="group overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1 cursor-pointer border-none bg-card/50 backdrop-blur">
                <CardContent className="p-8 text-center flex flex-col items-center">
                  <div className={`p-6 rounded-full ${cat.bg} mb-6 transition-transform group-hover:scale-110`}>
                  ...
                  <p className="text-muted-foreground">Premium quality {cat.title.toLowerCase()} products sourced directly from trusted local farmers.</p>
                </CardContent>
              </Card>
            </Link>
          ))}
        ...
        <div className="container mx-auto px-6 md:px-12 lg:px-20 grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div>
            <div className="flex justify-between items-center mb-8">
              ...
              <Link href="/news" className={buttonVariants({ variant: "ghost" })}>View All <ArrowRight className="ml-2 h-4 w-4" /></Link>
            </div>
              ...
              {[1, 2, 3].map((i) => (
                <Card key={i} className="hover:bg-muted/50 transition-colors">
                  <CardContent className="p-4 flex gap-4 items-center">
                    ...
                    <div>
                      <div className="text-sm text-primary font-medium mb-1">Market Update</div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="aspect-video bg-muted rounded-lg relative overflow-hidden group cursor-pointer">
